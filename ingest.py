...
def normalize_units(df: pd.DataFrame) -> pd.DataFrame:
    ...
    shelf_temp_c = _numeric_series(df, ["Shelf Temp (C)", "Shelf Temp (°C)", "Shelf Temp"])
    df["shelf_temp_k"] = shelf_temp_c + 273.15

    shelf_setpt_c = _numeric_series(df, ["Shelf Setpt (C)", "Shelf Setpt (°C)", "Shelf Setpt"])
    df["shelf_setpt_k"] = shelf_setpt_c + 273.15

    cond_temp_c = _numeric_series(df, ["Cond Temp (C)", "Cond Temp (°C)", "Cond Temp"])
    df["cond_temp_k"] = cond_temp_c + 273.15

    prod_avg_c = _numeric_series(df, ["Prod Avg (C)", "Prod Avg (°C)", "Prod Avg"])
    df["prod_avg_k"] = prod_avg_c + 273.15

    for tp in TP_COLUMNS:
        tp_c = _numeric_series(df, [f"{tp} (C)", f"{tp} (°C)", tp])
