...
def parse_calib(calib_str):
    ...
    for arg in calib_args:
        ...
        if vol_ul <= 0:
            raise ValueError(f"Invalid calibration volume in {arg!r}. Volume must be positive.")

        if height_mm <= 0:
            raise ValueError(f"Invalid calibration height in {arg!r}. Height must be positive.")
...
def choose_pressure_column(df):
    candidates = ["capman_pa", "pirani_pa", "vacuum_pa", "vac_setpt_pa"]
...
def choose_product_temperature_column(df, use_min=False):
    if use_min:
        ...
        tp_cols = [col for col in df.columns if col.lower().startswith("tp") and col.lower().endswith("_k")]
...
def build_segments(
    df, primary_phase_code, tube, fill_volume_m3, min_segment_points=5, hold_step_threshold=20,
):
    ...
    if primary_df.empty:
        available_phases = sorted(str(x) for x in df["Phase"].dropna().unique())
        raise ValueError(
            f"No rows found with Phase == {primary_phase_code!r}. "
            f"Available Phase values: {available_phases}"
        )

    model_columns = ["timestamp", "shelf_temp_k", "pressure_pa", "product_temp_k"]

    segments = []
    skipped = []

    for (cycle_id, step_id), group in primary_df.groupby(["Cycle", "Step"], sort=True):
        ...
        t_s = (group["timestamp"] - t0).dt.total_seconds().to_numpy(dtype=float)
